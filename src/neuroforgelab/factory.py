...
class SceneCfgFactory:
    ...
    def new_scene(
        ...
        cfg: InteractiveSceneCfg | None = None 
    ) -> InteractiveSceneCfg:
        ...
        for i, asset in enumerate([TerrainImporterCfg(terrain_type="generator", terrain_generator=self.terrain.to_cfg(), prim_path=f"/{self.name}/terrain")]):
            setattr(
                cfg,
                TERRAIN_NAME + f"_{i}",
                asset,
