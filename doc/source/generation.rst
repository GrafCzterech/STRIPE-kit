...
ensuring proper spacing and density. However, if you need to have your bushes
also be properly spaced, thus making their placement dependant (contextual),
...
nothing stopping you from making caves. Just maybe consider the computational
...
are lazy); expressed as the :py:attr:`stripe_kit.TerrainInstance.origin`.

Generally speaking, for generating terrain standard terrain procedural
...
does the sequence of events look like? Well first you define all the elements.
...
Well STRIPE-kit of course provides such a class:
...
Having created the training specification, in order to actually RL train, you
...
environment within gymnasium. In order to then train, we provide a convenience
